...
var logger = internal.GetLogger("S3meta")
...
func (m *RedisMeta) convertBucketName(bucketname string) string {
	return m.bucketPrefix + bucketname
}
func (m *RedisMeta) convertBucketNameBack(bucketname string) string {
	return strings.TrimPrefix(bucketname, m.bucketPrefix)
}

func (m *RedisMeta) GetBucketPattern() string {
...
func (m *RedisMeta) MakeBucket(bucket string) error {
	ctx := context.Background()
...
func (m *RedisMeta) DelBucket(bucket string) error {
	ctx := context.Background()
	bucket_in_redis := m.convertBucketName(bucket)
	logger.Infof("RedisMeta::DelBucket[%s]", bucket_in_redis)

	return m.rdb.Del(ctx, bucket_in_redis, "").Err()
}

func (m *RedisMeta) ListBuckets() ([]minio.BucketInfo, error) {
	ctx := context.Background()
	...
	for iter.Next(ctx) {
		...
		logger.Infof("RedisMeta::ListBuckets: find bucket name:%s", bucketinfo.Name)
	}

	return result, nil
}

func (m *RedisMeta) ListObjects(bucket string) ([]minio.ObjectInfo, error) {
	/*ctx := context.Background()
...
func (m *RedisMeta) PutObjectMeta(object minio.ObjectInfo) error {
	ctx := context.Background()
	jsondata, err := json.Marshal(object)
	if err != nil {
		return err
	}
	return m.rdb.SAdd(ctx, m.objectKey(object), jsondata).Err()
}

func (m *RedisMeta) GetObjectMeta(object minio.ObjectInfo) error {
	ctx := context.Background()

	obj_info, err := m.rdb.SMembers(ctx, m.objectKey(object)).Result()
	if err != nil {
		return err
	}
	if len(obj_info) == 0 {
	...
	return json.Unmarshal([]byte(obj_info[0]), &object)
}
