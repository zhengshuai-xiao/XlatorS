...
func TestManifestOperations(t *testing.T) {
	...
	originalManifest := []ChunkInManifest{
		{FP: padFP("fp1-string-for-32-bytes-length"), Len: 1024, DCID: 1},
		{FP: padFP("fp2-string-for-32-bytes-length"), Len: 2048, DCID: 2},
		{FP: padFP("fp3-string-for-32-bytes-length"), Len: 4096, DCID: 1},
	}

	t.Run("WriteAndReadManifest", func(t *testing.T) {
		// --- Write Manifest ---
		uniqueDCIDs, err := xlator.writeManifest(ctx, namespace, manifestID, originalManifest)
		assert.NoError(t, err)
		assert.ElementsMatch(t, []uint64{1, 2}, uniqueDCIDs)
	...
	t.Run("ReadUniqueDCIDs", func(t *testing.T) {
		// File should exist from previous test
		dcids, err := xlator.readUniqueDCIDs(ctx, namespace, manifestID)
		assert.NoError(t, err)
		assert.ElementsMatch(t, []uint64{1, 2}, dcids)
	})
