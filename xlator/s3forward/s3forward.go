...
const (
	...
	nullVersionID = "null"
	XlatorName    = "S3Forward"
)

var logger = internal.GetLogger("s3forward")

func NewS3Object(gConf *internal.Config) (*s3Objects, error) {
	if gConf.Xlator != XlatorName {
		logger.Fatalf("Invalid S3 Gateway configuration(%s!=%s)", gConf.Xlator, XlatorName)
		return nil, fmt.Errorf("Invalid S3 Gateway configuration(%s!=%s)", gConf.Xlator, XlatorName)
	}
...
func (l *s3Objects) StorageInfo(ctx context.Context) (si minio.StorageInfo, _ []error) {
	logger.Infof("%s: enter", internal.GetCurrentFuncName())
	si.Backend.Type = internal.BackendGateway
...
func (l *s3Objects) SetBucketPolicy(ctx context.Context, bucket string, bucketPolicy *policy.Policy) error {
	...
	if err != nil {
		...
		logger.Infof("failed to marshal bucket policy: %v", err)
		return minio.ErrorRespToObjectError(err, bucket)
	}
